			...
			<Flex h={'full'} justify={'space-between'} align={'center'}>
				...
				<HStack>
					...
					<Menu placement='bottom'>
						...
						<MenuList>
							{ language.map(item =>(
								<MenuItem  onClick={() => onLanguage(item.lng)} key={item.lng} icon={<item.icon/>}
								background={i18n.resolvedLanguage === item.lng ? "facebook.500":''}
								>{item.nativeLng} </MenuItem>
							))}
					...
					<IconButton
						...
						variant={'outline'}
					/>
					<Button rightIcon={<BiUserCircle />} colorScheme={'facebook'} onClick={()=> router.push("/auth")} >
						{ t('login',{ns:'layout'})}
					</Button>
				</HStack>
			</Flex>
